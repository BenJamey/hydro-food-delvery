...
def add_restaurant_image(restaurant_id):
    ...
    if request.method == 'POST':
        try:
            # Handle image upload - FIXED: now matches HTML form name
            if 'restaurant_image' in request.files:  # Changed to match HTML
    ...
    # FIXED: Get restaurant data to pass to template
    restaurant = conn.execute('SELECT * FROM restaurants WHERE id = ?', (restaurant_id,)).fetchone()
    conn.close()
    ...
    return render_template('Add_rest_image.html', restaurant=restaurant, restaurant_id=restaurant_id)
