...
def register():
    if request.method == 'POST':
        ...
        elif isAdmin == 1: #If th new user is an admin user, they need to enter a special key to sign up
...
# The admin user wishes to see all the users and they're deails
@app.route('/view_users')
@admin_required # Only admin users can see all the users in the data base
def view_users():
...
# Called when tyhe admin user wishes to delete an user from the list 
@app.route('/delete_user/<int:user_id>', methods=['POST'])
@admin_required # Only admin users can delete users from the database
...
#Loads the restauraunts homepage
@app.route('/')
def homepage():
    try:
        conn = get_db_connection()
...
# Used when adding a new restauraunt
...
# Used to edit an existing restauraunt
...
# Used to add image to restauraunt
...
# Used to add image to an existing restauraunt
...
# Loads the menu where the user can upload an imagre for a menu item
...
# User when the user wishes to delete an item from the menu
...
# User when the user wishes to edit an item from the menu
