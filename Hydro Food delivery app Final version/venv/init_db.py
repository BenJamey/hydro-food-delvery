...
def init_db():
    ...
    user_count = cursor.execute('SELECT COUNT(*) FROM users_list').fetchone()[0]
    ...
    if user_count == 0:
        ...
        user_data = [
            ('User1', hash_password('P@$$word123'), 'User1@gmail.co.nz', '123 Queen Street, Auckland, New Zealand', 0),  # Regular user
            ('AdminUser', hash_password('ADM_P@SS'), 'AdminUSE@gmail.co.nz', '456 Albert Street, Auckland, New Zealand', 1)  # Admin user
        ]
        ...
        print(f"Added {len(user_data)} user accounts.")
    else:
        print(f"Database already contains {user_count} users. Skipping user data insertion.")
